...
### Optional — Social Sign-In (API only; each provider's button stays "Coming soon" until its variables are all set)
```
GOOGLE_CLIENT_ID=...
...
APPLE_KEY_ID=...
APPLE_PRIVATE_KEY=...
APPLE_REDIRECT_URI=https://api.example.com/api/v1/auth/apple/callback
